...
public class EspecialidadController {
	...
	/*
	@PostMapping	
	public ResponseEntity<EspecialidadDTO> registrar(@Valid @RequestBody EspecialidadDTO dtoRquest) throws Exception{
		Especialidad p = mapper.map(dtoRquest, Especialidad.class);
	...
	@PostMapping	
	public ResponseEntity<Void> registrar(@Valid @RequestBody EspecialidadDTO dtoRquest) throws Exception{
		Especialidad p = mapper.map(dtoRquest, Especialidad.class);
		Especialidad obj = service.registrar(p);
		EspecialidadDTO dtoResponse = mapper.map(obj, EspecialidadDTO.class);
	...
	@PutMapping	
	public ResponseEntity<EspecialidadDTO> modificar(@Valid @RequestBody EspecialidadDTO dtoRquest) throws Exception{
		Especialidad pac = service.listarPorId(dtoRquest.getIdEspecialidad());
		if (pac == null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO" + dtoRquest.getIdEspecialidad());
		}
	...
	public EntityModel<EspecialidadDTO> listarHateoas(@PathVariable("id") Integer id) throws Exception{
		...
		//localhost:8094/Especialidades/1

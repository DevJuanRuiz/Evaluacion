...
public class MedicoController {
	...
	/*
	@PostMapping	
	public ResponseEntity<MedicoDTO> registrar(@Valid @RequestBody MedicoDTO dtoRquest) throws Exception{
		Medico p = mapper.map(dtoRquest, Medico.class);
	...
	@PostMapping	
	public ResponseEntity<Void> registrar(@Valid @RequestBody MedicoDTO dtoRquest) throws Exception{
		Medico p = mapper.map(dtoRquest, Medico.class);
		Medico obj = service.registrar(p);
		MedicoDTO dtoResponse = mapper.map(obj, MedicoDTO.class);
	...
	@PutMapping	
	public ResponseEntity<MedicoDTO> modificar(@Valid @RequestBody MedicoDTO dtoRquest) throws Exception{
		Medico pac = service.listarPorId(dtoRquest.getIdMedico());
		if (pac == null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO" + dtoRquest.getIdMedico());
		}
